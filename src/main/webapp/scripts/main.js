(function() {
	...
	function loadRecommendedItems() {
		...
		// make AJAX call
		ajax('GET', url, data, 										//The only difference
				// successful callback
				function(res) {
					var items = JSON.parse(res);
					if (!items || items.length === 0) {
	...
	function register() {
		...
		//use md5 to improve security加密
		password = md5(username + md5(password));

		// The request parameters -- ajax
		var url = './register';
		var req = JSON.stringify({
			user_id : username,
			password : password,
	...
	init();//The whole JS only execute this line. All logic is with this function
